...
import config_atbench_trace_v3 as C
from stage_a_atbench_trace import (
    MechanismDiscovery,
    split_path,
    load_json,
)
...
def _prefer_traj_label(df: pd.DataFrame) -> pd.DataFrame:
    """
    If a table has both event-level label and traj_label, use traj_label as label.
    For the recommended ATBench path, step_df.pkl already has trajectory-level label.
    """
    df = df.copy()
...
def normalize_existing_step_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Recommended for in-domain ATBench:
      pass C.OUT_DIR/step_df.pkl, which already contains x with the same dimension
      used during Stage-B training.
    """
    df = df.copy()
    ...
    if "x" in df.columns:
        ...
        out = df.sort_values(["traj_id", "event_order"]).reset_index(drop=True)
        print("[normalize] using existing x column")
        print("[normalize] x_dim:", np.asarray(out["x"].iloc[0]).shape[0])
        print("[normalize] rows:", len(out))
        print("[normalize] trajs:", out["traj_id"].nunique())
    ...
    print("[normalize] built x from raw rep/layer table")
    print("[normalize] layers:", C.LAYERS)
...
def infer_ckpt_input_dim(model_ckpt: dict) -> Optional[int]:
    if "input_dim" in model_ckpt:
        try:
            return int(model_ckpt["input_dim"])
        except Exception:
            pass

    state = model_ckpt.get("model_state", {})

    # raw_proj.0 is LayerNorm(input_dim)
    if "raw_proj.0.weight" in state:
        return int(state["raw_proj.0.weight"].shape[0])

    # raw_proj.1 is Linear(input_dim -> raw_dim)
    if "raw_proj.1.weight" in state:
        return int(state["raw_proj.1.weight"].shape[1])

    return None


def check_dimensions_or_raise(
    step_input_dim: int,
    model_ckpt: dict,
    stage_a_ckpt: dict,
    args,
):
    ckpt_input_dim = infer_ckpt_input_dim(model_ckpt)
    stage_a_input_dim = int(stage_a_ckpt["input_dim"])

    print("[dim check]")
    print(f"  step/table input_dim = {step_input_dim}")
    print(f"  stage_a input_dim    = {stage_a_input_dim}")
    print(f"  stage_b ckpt input_dim = {ckpt_input_dim}")

    errors = []

    if step_input_dim != stage_a_input_dim:
        errors.append(
            f"step/table input_dim ({step_input_dim}) != stage_a input_dim ({stage_a_input_dim})"
        )

    if ckpt_input_dim is not None and step_input_dim != ckpt_input_dim:
        errors.append(
            f"step/table input_dim ({step_input_dim}) != stage_b checkpoint input_dim ({ckpt_input_dim})"
        )
    ...
    if errors:
        ...
        raise RuntimeError(
            ...
            f"{msg}\n\n"
            "Most likely cause:\n"
            "  The evaluator is reading a table built with different C.LAYERS than the one used for Stage B training.\n"
            "For Qwen3-4B, one layer is often 2560 dims. Five layers give 12800 dims.\n\n"
            "Recommended fix:\n"
            "  Use the exact Stage-A output step_df.pkl from the same OUT_DIR as the Stage-B checkpoint:\n"
            f"    --table_path {os.path.join(os.path.dirname(args.ckpt), 'step_df.pkl')}\n"
            f"    --stage_a_ckpt {os.path.join(os.path.dirname(args.ckpt), 'stage_a_latent_mechanism.pt')}\n"
            "  and pass the matching split file with --split_path.\n"
        )
...
def load_model(args, input_dim: int):
    ...
    valid_model_names = {
        "trajectory_state_auditor_v3",
        "atbench_trajectory_state_auditor_v3",
        "asse_strict_trajectory_state_auditor_v3",
    }

    model_name = model_ckpt.get("config", {}).get("model", None)

    if model_name is not None and model_name not in valid_model_names:
        print(
            f"[warning] checkpoint config model={model_name}; "
            f"this evaluator expects a Stage-B V3 trajectory-state auditor."
        )

    check_dimensions_or_raise(
        step_input_dim=input_dim,
...
def early_warning_metrics(prefix_step_df: pd.DataFrame, threshold: float):
    ...
    for tid, g in df.groupby("traj_id"):
        ...
        # Any early alert, including false positives.
        if T > 1:
    ...
    def add_stats(prefix, frame, advance_col, rel_col):
        if len(frame) > 0:
            out[f"{prefix}_mean_advance_steps_detected_only"] = float(frame[advance_col].mean())
            out[f"{prefix}_median_advance_steps_detected_only"] = float(frame[advance_col].median())
            out[f"{prefix}_mean_relative_detect_detected_only"] = float(frame[rel_col].mean())
            out[f"{prefix}_median_relative_detect_detected_only"] = float(frame[rel_col].median())
        else:
            out[f"{prefix}_mean_advance_steps_detected_only"] = float("nan")
            out[f"{prefix}_median_advance_steps_detected_only"] = float("nan")
            out[f"{prefix}_mean_relative_detect_detected_only"] = float("nan")
            out[f"{prefix}_median_relative_detect_detected_only"] = float("nan")

    add_stats("first_detect", first_df, "advance_steps", "relative_detect")
    add_stats("first_early_detect", first_early_df, "advance_steps", "relative_detect")
    add_stats("stable_detect", stable_df, "stable_advance_steps", "relative_stable_detect")
    add_stats("stable_early_detect", stable_early_df, "stable_advance_steps", "relative_stable_detect")
...
def load_eval_step_df(args):
    ...
    out = step_df[step_df["traj_id"].isin(ids)].copy()
    print(f"[split] mode={args.mode}, selected trajs={out['traj_id'].nunique()}, rows={len(out)}")
...
def parse_args():
    ...
    ap.add_argument(
        ...
        default=None,
        required=True,
        help="Stage-B V3 checkpoint, e.g. atbench_trajectory_state_auditor_v3.pt",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="In-domain step_df path. Default: C.OUT_DIR/step_df.pkl. Recommended: use the same step_df.pkl generated by Stage A.",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Split json path. Default: split_path() from stage_a_atbench_trace.py",
    )
