...
def split_path():
    return os.path.join(
        C.OUT_DIR,
        getattr(C, "SPLIT_FILENAME", "atbench_train_val_test_split_ids.json"),
    )
...
def get_obj_traj_id(obj, fallback_idx):
    """
    ATBench jsonl may use traj_id / trajectory_id / id / idx / index.
    If none exists, use line index.
    """
...
def normalize_binary_label(y):
    """
    Normalize common ATBench label formats into:
      0 = safe
      1 = unsafe
    """
    ...
    if isinstance(y, float):
        if int(y) in [0, 1] and float(y) == float(int(y)):
            return int(y)

    if isinstance(y, str):
        yy = y.strip().lower()
        if yy in ["0", "safe", "benign", "normal", "harmless"]:
            return 0
        if yy in ["1", "unsafe", "risky", "harmful", "attack", "malicious"]:
            return 1

    raise ValueError(f"Cannot normalize binary label: {y}")


def load_traj_label_map(jsonl_path):
    """
    Critical logic:
      Use top-level trajectory label from ATBench trajectory jsonl.
      Then broadcast this trajectory-level label to every step.

    Expected default:
      obj["label"] in {0, 1}, where 1 = unsafe.

    You can override the label key in config:
      LABEL_KEY = "label"
    """
    rows = load_jsonl(jsonl_path)
    label_key = getattr(C, "LABEL_KEY", "label")
    ...
    for i, obj in enumerate(rows):
        ...
        if label_key not in obj:
            raise KeyError(
                f"Trajectory object at line {i} has no top-level '{label_key}'. "
                f"Available keys={list(obj.keys())}"
            )

        y = normalize_binary_label(obj[label_key])
        label_map[tid] = y
...
def load_step_df():
    ...
    if len(step_df) == 0:
        raise RuntimeError(
            "No valid step rows were built. Please check TABLE_PATH, TRAJ_JSONL_PATH, "
            "EVENT_TYPE, LAYERS, and traj_id alignment."
        )
...
def create_or_load_split(
    ...
    if test_ratio > 0:
        train_val_ids, test_ids, train_val_y, test_y = train_test_split(
            ...
            random_state=seed,
            stratify=labels if stratify_by_label else None,
        )
    else:
        train_val_ids = traj_ids
        train_val_y = labels
        test_ids = np.array([], dtype=traj_ids.dtype)

    val_ratio_within_train_val = val_ratio / max(train_ratio + val_ratio, 1e-12)
    ...
    train_ids, val_ids, train_y, val_y = train_test_split(
        ...
        random_state=seed,
        stratify=train_val_y if stratify_by_label else None,
    )
...
def train_stage_a(args):
    ...
    print(
        f"[data] train steps={len(train_df)}, "
        f"val steps={len(val_df)}, "
        f"test steps={len(test_df)}"
    )
    print(
    ...
    print(f"Saved ATBench Stage A to {ckpt_path}")
