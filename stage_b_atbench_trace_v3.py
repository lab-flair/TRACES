    ...
    for tid, g in step_df.groupby("traj_id", sort=False):
        ...
        if len(set(labels.tolist())) != 1:
            raise RuntimeError(
                ...
                f"Make sure Stage A broadcasts trajectory-level labels to all steps."
            )
...
class TrajectoryStateAuditor(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        return {
            ...
            # compatibility names for old evaluator
...
def handle_non_constant_labels(step_df):
    ...
    if not C.DROP_NON_CONSTANT_LABEL_TRAJS:
        raise RuntimeError(
            ...
            f"Set DROP_NON_CONSTANT_LABEL_TRAJS=True to drop them, "
            f"or fix Stage A label broadcast."
        )
...
def train(args):
    ...
    print("[Stage B V3] ATBench trajectory-state auditor with delta features")
    ...
    if not os.path.exists(step_df_path):
        raise FileNotFoundError(
            f"{step_df_path} not found. Run ATBench Stage A first."
        )
    ...
    if not os.path.exists(split_file):
        raise FileNotFoundError(
            f"{split_file} not found. Run ATBench Stage A split creation first."
        )
    ...
    torch.save(
        {
            ...
            "config": {
                "model": "atbench_trajectory_state_auditor_v3",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_prefix",
        type=str,
        default="atbench_trace_temporal_state_v3",
    )
